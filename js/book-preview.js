...
(function () {
  ...
  function _wireSwipe() {
    ...
    card.addEventListener('touchmove',   _onTouchMove,   { passive: true });
  ...
  function renderContent(b, meta, detail, mode) {
    ...
    if (mode === 'library') {
      function _statusBtnStyle(key) {
        ...
        return `flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 4px;border-radius:12px;cursor:pointer;font-size:12px;font-weight:700;white-space:nowrap;font-family:var(--font-body,inherit);background:${s.bg};color:${s.fg};border:${active ? `2px solid ${s.fg}` : '2px solid transparent'};box-shadow:${active ? `0 0 0 1px ${s.fg}44` : 'none'}`;
      }
      primaryBtn = `
        <div style="width:100%">
        ...
        <div style="width:100%">
          ...
          <div style="display:flex;gap:8px">
            <button type="button" id="previewSaveWantBtn"
              style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 4px;border-radius:12px;border:none;background:${STATUS_CFG.want.bg};color:${STATUS_CFG.want.fg};font-size:12px;font-weight:700;white-space:nowrap;cursor:pointer;font-family:var(--font-body,inherit)">
              ${svgIcon('want', 18)}읽을 책
            </button>
            <button type="button" id="previewSaveReadingBtn"
              style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 4px;border-radius:12px;border:none;background:${STATUS_CFG.reading.bg};color:${STATUS_CFG.reading.fg};font-size:12px;font-weight:700;white-space:nowrap;cursor:pointer;font-family:var(--font-body,inherit)">
              ${svgIcon('reading', 18)}읽는 중
            </button>
            <button type="button" id="previewSaveBtn" data-save-status=""
              style="flex:1;display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 4px;border-radius:12px;border:none;background:${STATUS_CFG.read.bg};color:${STATUS_CFG.read.fg};font-size:12px;font-weight:700;white-space:nowrap;cursor:pointer;font-family:var(--font-body,inherit)">
              ${svgIcon('read', 18)}읽은 책
            </button>
          </div>
        </div>`;
  ...
  async function handleFindLibrary(isbn, btn) {
    ...
    try {
      ...
      if (needsRegion) {
        const regions = window.LibraryAPI.getRegionOptions ? window.LibraryAPI.getRegionOptions() : [];
        const preferred = inferPreferredRegionCode(regions);
        const options = regions.map(r =>
          `<option value="${escapeAttr(r.code)}" ${r.code === preferred ? 'selected' : ''}>${escapeHtml(r.name)}</option>`
        ).join('');
        section.innerHTML = `
          <div style="padding:14px;background:rgba(0,0,0,0.04);border-radius:8px;text-align:center">
            <div style="font-size:13px;color:var(--text-soft);margin-bottom:10px">위치 권한 없이도<br>지역으로 도서관을 찾을 수 있어요</div>
            <div style="display:flex;gap:6px;margin-bottom:8px">
              <select id="previewLibraryRegion" style="flex:1;min-width:0;padding:8px 10px;border:1.5px solid rgba(45,106,79,0.25);border-radius:6px;background:#fff;color:var(--text);font-size:12px">
                ${options}
              </select>
              <button type="button" id="previewRegionLibraryBtn"
                style="padding:8px 12px;border:none;border-radius:6px;background:#2d6a4f;color:#fff;font-size:12px;font-weight:700;cursor:pointer">
                찾기
              </button>
            </div>
            <button type="button" id="previewFindLibraryBtn"
              style="padding:6px 12px;border:none;background:transparent;color:#2d6a4f;font-size:11px;font-weight:600;cursor:pointer">
              📍 위치로 다시 시도
            </button>
          </div>`;
        ...
        const regionBtn = section.querySelector('#previewRegionLibraryBtn');
        if (regionBtn) regionBtn.addEventListener('click', () => handleFindLibraryByRegion(isbn, regionBtn));
        return;
      }

      renderLibraryResults(section, libraries, regionName);
    } catch (e) {
      section.innerHTML = `
  ...
  async function handleFindLibraryByRegion(isbn, btn) {
    const section = document.getElementById('previewLibrarySection');
    const select = document.getElementById('previewLibraryRegion');
    if (!section || !select || !window.LibraryAPI) return;

    btn.disabled = true;
    btn.textContent = '검색 중…';

    try {
      const { libraries, regionName } = await LibraryAPI.findLibrariesByRegion(isbn, select.value);
      renderLibraryResults(section, libraries, regionName);
    } catch (e) {
      section.innerHTML = `
        <div style="padding:12px;background:rgba(0,0,0,0.04);border-radius:8px;text-align:center;font-size:12px;color:var(--text-mute)">
  ...
  function renderLibraryResults(section, libraries, regionName) {
    if (!libraries.length) {
      section.innerHTML = `
  ...
  function inferPreferredRegionCode(regions) {
    const saved = [
      localStorage.getItem('profileCity'),
      localStorage.getItem('profileRegion')
    ].filter(Boolean).join(' ');
    const hit = regions.find(r => saved.includes(r.name));
    return hit ? hit.code : (regions[0] && regions[0].code);
  }
