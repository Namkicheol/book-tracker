# book-tracker — CLAUDE.md (Claude Code)

> 이 파일이 이 레포의 canonical agent instruction이다. Claude Code는 작업 시작 시 이 파일을 우선 참고한다. Codex 진입 문서는 `AGENTS.md`이고, 두 파일은 도구 framing만 다르고 섹션·순서·규칙은 동일하다.
> 새 규칙은 양쪽 모두 갱신한다.
...
4. feature 브랜치를 만들 때 Claude Code 기본 prefix는 `claude/`를 사용한다.
...
- Claude Code 진입 문서는 이 `CLAUDE.md`다. `AGENTS.md`는 Codex용 parallel 문서.
- 로컬 웹 확인이 필요하면 정적 서버를 띄운 뒤 Playwright MCP 또는 브라우저로 확인한다.
