#!/usr/bin/env python3
"""Round-8 loader: round7 eligible 풀(/tmp/wb6_eligible.json)에서 다음 배치 선별
→ 알라딘 Worker/카카오로 표지·메타 보강. 재스크레이프 없음.

성인/YA 프랜차이즈·성숙 콘텐츠 제외(MATURE) 필터 유지 (#49 교훈).
타겟: band0=18, band1~5=14 (현재 680권 분포상 band0 최저).

Run from repo root: python3 scripts/wendybook_bands_round8_en.py
"""
...
ELIG_IN = '/tmp/wb7_eligible.json'
CKPT = '/tmp/wb8_enriched.json'
ELIG_OUT = '/tmp/wb8_eligible.json'
...
def main():
    ...
    print(f'Remaining eligible for round 9: {len(remaining)} -> {ELIG_OUT}')
