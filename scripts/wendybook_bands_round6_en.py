#!/usr/bin/env python3
"""Round-6 loader: 웬디북 eligible 풀(round5가 저장한 /tmp/wb5_eligible.json)에서
다음 배치 선별 → 알라딘 Worker/카카오로 표지·메타 보강. 재스크레이프 없음.

추가 필터(round5 대비): 성인/YA 프랜차이즈·성숙 콘텐츠 제외(MATURE).
  - AR 지수(텍스트 난이도) ≠ 연령 적합성. #49에서 GoT/AGGGTM/Hunger Games 제거한 교훈 반영.

저밴드 보강 타겟: band0=20, band1=15, band2~5=13 (현재 593권 분포상 band0/1 최저).

Run from repo root: python3 scripts/wendybook_bands_round6_en.py
"""
...
ELIG_IN = '/tmp/wb5_eligible.json'
CKPT = '/tmp/wb6_enriched.json'
ELIG_OUT = '/tmp/wb6_eligible.json'

DROP_ISBN = {'9780593202364'}  # All My Rage (성숙 YA)
MATURE = re.compile(
...
TARGET = {0: 20, 1: 15, 2: 13, 3: 13, 4: 13, 5: 13}
...
def main():
    ...
    print(f'Remaining eligible for round 7: {len(remaining)} -> {ELIG_OUT}')
