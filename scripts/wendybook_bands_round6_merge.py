#!/usr/bin/env python3
"""Round-6 merge: /tmp/wb6_enriched.json -> data/book-recommendations.json

기존 EN 스키마와 정확히 일치(round5 정정본과 동일):
  (isbn, title, author, publisher, year, ar, language, lexile, thumbnail, genre, lists, why)
- thumbnail/lexile 보존, targetAge 미추가, 기존 키 순서 보존(재정렬 금지) → 끝에 append.
- 제목 내 한글 부기 제거(예: "(흑백)"), 이중 공백 정리.

Run from repo root: python3 scripts/wendybook_bands_round6_merge.py
"""
import json, os, re
